...
| 先决条件： | HTML基础知识（研究[HTML简介），以及有关CSS如何工作的想法（研究[CSS和[样式框[简介）。 |
| :--------- | ------------------------------------------------------------ |
...
大多数遗留框架使用该[`float`]( /float)属性的行为将一列浮动到另一列，以创建看起来像网格的内容。通过创建带有浮点数的网格的过程，可以向您展示这是如何工作的，还介绍了一些更高级的概念，以您在[浮点数和清除的课程中学到的东西为基础。
...
我们的网格工作良好，但是具有固定的宽度。我们确实想要一个灵活的（流体）网格，该网格将随着浏览器[视口中的可用空间而增长和缩小。为此，我们可以将参考像素宽度转换为百分比。
...
如果您阅读了有关[flexbox的上一篇文章，您可能会认为flexbox是创建网格系统的理想解决方案。有很多基于Flexbox的网格系统可用，Flexbox可以解决我们在上面创建网格时已经发现的许多问题。
