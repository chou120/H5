HTML的主要工作是编辑文本结构和文本内容（也称为语义[semantics]( 1/docs/Glossary/语义)），以便浏览器能正确的显示。 本文介绍了 [HTML]( 1/docs/Glossary/HTML)的使用方法：在一段文本中添加标题和段落，强调语句，创建列表等等。

| 先决条件: | 阅读 [开始学习 HTML]( 1/docs/Learn/HTML/Introduction_to_HTML/Getting_started)，了解基本的 HTML 知识。 |
| :-------- | ------------------------------------------------------------ |
...
- 对您的网页建立索引的搜索引擎将标题的内容视为影响网页搜索排名的重要关键字。没有标题，您的网页在[SEO]( 1/docs/Glossary/SEO)（搜索引擎优化）方面效果不佳。
...
- 使用[CSS]( 1/docs/Glossary/CSS)样式化内容，或者使用[JavaScript]( 1/docs/Glossary/JavaScript)做一些有趣的事情，你需要包含相关内容的元素，所以CSS / JavaScript可以有效地定位它。
...
这里是最好的经验法则：使`<b>`,`<i>`,`<u>` 来传达传统意义上的粗体，斜体或下划线是合适的，没有其他元素更适合这样用了。然而，始终拥有[可访问性]( 1/docs/learn/Accessibility)的思维模式是至关重要的。斜体的概念对人们使用屏幕阅读器是没有帮助的，对使用其他书写系统而不是拉丁文书写系统的人们也是没有帮助的。
