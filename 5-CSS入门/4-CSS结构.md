...
在CSS入门中,我们将外部样式表链接到了我们的页面。这是将CSS附加到文档的最常用和最有用的方法，因为您可以将CSS链接到多个页面，从而使您可以使用相同的样式表对它们进行样式设置。在大多数情况下，网站的不同页面看起来几乎都相同，因此您可以为基本外观使用相同的规则集。
...
没有开会选择器就无法谈论CSS，我们已经在CSS入门教程中发现了几种不同的类型。选择器是我们如何在HTML文档中定位某些对象以便对其应用样式。如果您的样式没有应用，则您的选择器可能与您认为应该匹配的内容不匹配。
...
专门性规则和级联规则乍一看似乎有些复杂，一旦您掌握了更多的CSS知识，就更容易理解。在我们的下一个级联和继承文章中，您将在下一个模块中找到该文章，我将对此进行详细说明，包括如何计算特异性。现在，请记住这已经存在，并且有时CSS可能不适用于您期望的样式，因为样式表中的其他内容具有更高的特异性。识别一个以上的规则可能适用于某个元素是解决此类问题的第一步。
...
另一个示例是的各种值`transform`，例如`rotate()`。
...
- **`transform`**
- **`background-image`，尤其是渐变值**
- **`color`，尤其是rgb / rgba / hsl / hsla值**
...
`@rules`您会遇到的最常见的情况之一是`@media`，它允许您仅在满足某些条件时（例如，当屏幕分辨率超过一定数量或屏幕宽度超过一定宽度时），使用媒体查询来应用CSS。
...
一些属性，如`font`，`background`，`padding`，`border`，和`margin`被称为**速记性能** -这是因为它们允许你在一个单行设置几个属性值，节省了时间，让您的代码更简洁的过程中。
