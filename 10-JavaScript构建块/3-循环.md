...
让我们开始探索一些特定的循环结构。第一个是[for循环，您将在大多数时候使用它，它具有以下语法：
...
将[继续以类似的方式来陈述的作品`break`，但不是破环了全部，它跳过到循环的下一次迭代。让我们看另一个示例，该示例以数字作为输入，并且仅返回整数（整数）平方的数字。
...
2. 在循环内，我们使用[Math.sqrt（i）找到每个数字的平方根，然后通过测试平方根是否舍入为最接近的整数（此整数是否相同[）来检查平方根是否为整数是[Math.floor（）对传递的数字所做的操作）。
...
额外的奖励问题-成功完成上述任务后，您将获得两个名称列表，以逗号分隔，但它们不整洁-每个名称的末尾都有一个逗号。您能算出如何编写在每种情况下都将最后一个逗号切开的行，并在末尾添加句号吗？请参阅“ [有用的字符串方法”文章以寻求帮助。
