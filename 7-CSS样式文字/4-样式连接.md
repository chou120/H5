在对[链接进行样式设置时，重要的是要了解如何有效地使用伪类来对链接状态进行样式设置，以及如何对链接进行样式设置以用于常见的各种界面功能（例如导航菜单和选项卡）。我们将在本文中介绍所有这些主题。
...
我们根据[创建超链接中的最佳实践研究了如何在HTML中实现[链接。在本文中，我们将基于这些知识，向您展示样式链接的最佳实践。
...
- **Focus**：链接已被聚焦时的链接（例如，键盘用户使用Tab键或类似键移动到该链接，或者使用来以编程方式聚焦[`HTMLElement.focus()`）-使用[`:focus`]( /:focus)伪类设置了链接的样式。
...
最后一句话-我们如何仅选择外部链接？好吧，如果正确编写[HTML链接，则只应使用绝对URL作为外部链接-使用相对链接来链接到您自己网站的其他部分会更有效。因此，文本“ http”应仅出现在外部链接中，并且我们可以使用[属性选择器进行选择]( 1

就是这样-尝试重新访问上面的主动学习部分，然后尝试这项新技术！

**注意**：如果您还不熟悉[背景和[自适应网页设计，**请**不要担心；这些在其他地方有解释。
