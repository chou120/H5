...
与之前的文章一样，让我们通过在[浏览器开发人员控制台中输入一些示例来了解数组的真正基础。
...
3. 在`// number 3`注释的正下方，我们希望您编写一行代码，将当前数组项目（`name:price`）分为两个单独的项目，一个仅包含名称，而另一个仅包含价格。如果不确定如何执行此操作，请查阅“ [有用的字符串方法”文章以寻求帮助，甚至更好的是，请[参见本文的“在字符串和数组之间转换”](1/en-US/docs/Learn/JavaScript/First_steps/Arrays#Converting_between_strings_and_arrays)部分。
