在本课程的这一点上，我们将讨论JavaScript中的数学-如何使用运算符和其他功能来成功地操纵数字来进行出价。
...
- **二进制** -计算机的最底层语言；0和1。
- **八进制** —以8为底，每列使用0–7。
- **十六进制** —以16为底，在每列中使用0–9，然后使用a–f。在CSS中
...
**注意**：实际上，JavaScript具有第二个数字类型BigInt，用于非常大的整数。但是出于本课程的目的，我们将只考虑`Number`价值。

### 这都是我的数字



让我们快速处理一些数字，以重新了解所需的基本语法。在开发人员工具JavaScript控制台中输入下面列出的命令。
...
有时，您可能想在数字变量值之间反复添加或减去一个。可以使用增量（`++`）和减量（`--`）运算符方便地完成此操作。我们用`++`我们的“猜数字”游戏早在我们的第一章入门JavaScript文章中，我们添加1到我们的`guessCount`变量来跟踪用户有多少猜测每个回合后离开。

```js
guessCount++;
```

**注意**：这些运算符最常在循环中使用，您将在本课程的后面部分中进行了解。例如，假设您要遍历价格清单，并为每个清单增加营业税。您将使用循环依次遍历每个值，并进行必要的计算以增加每种情况下的营业税。增量器用于在需要时移至下一个值。
